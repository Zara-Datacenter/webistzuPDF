...
const CACHE_VERSION = 'webistzupdf-v7';
            ...
            .then((cacheNames) => {
                return Promise.all(
                    cacheNames.map((cacheName) => {
                        if (cacheName.startsWith('webistzupdf-') && cacheName !== CACHE_NAME) {
...
function getLocalPathForCDNUrl(pathname) {
    if (pathname.includes('/@webistzupdf/pymupdf-wasm')) {
        return '/pymupdf-wasm/';
    }
    if (pathname.includes('/@webistzupdf/gs-wasm')) {
...
function shouldCache(pathname, isCDN = false) {
    if (isCDN) {
        return (
            pathname.includes('/@webistzupdf/pymupdf-wasm') ||
            pathname.includes('/@webistzupdf/gs-wasm') ||
