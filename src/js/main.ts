...
const init = async () => {
  ...
  if (__SIMPLE_MODE__) {
    const hideBrandingSections = () => {
      ...
      if (nav) {
        ...
        simpleNav.className =
          'bg-slate-900 border-b border-slate-800 sticky top-0 z-30';
        simpleNav.innerHTML = `
          <div class="container mx-auto px-4">
            <div class="flex justify-start items-center h-16">
              <div class="flex-shrink-0 flex items-center cursor-pointer" id="home-logo">
                <img src="/logo.png" alt="WebistzuPDF Logo" class="h-10 w-auto">
                <span class="text-white font-bold text-xl ml-2">
                  <a href="index.html">WebistzuPDF</a>
                </span>
              </div>
            </div>
          </div>
      ...
      const githubLink = document.querySelector(
        'a[href*="github.com/Zara-Datacenter/webistzupdf"]'
      );
      if (githubLink) {
      ...
      if (footer && !document.querySelector('[data-simple-footer]')) {
        ...
        simpleFooter.className = 'mt-16 border-t-2 border-slate-800 py-8';
            ...
            <div class="flex items-center justify-between flex-wrap gap-4">
              <div>
                <div class="flex items-center mb-2">
                  <img src="/logo.png" alt="WebistzuPDF Logo" class="h-10 w-auto mr-2">
                  <span class="text-white font-bold text-lg">WebistzuPDF</span>
                </div>
                <p class="text-gray-400 text-sm">
                  &copy; 2026 WebistzuPDF. All rights reserved.
                </p>
      ...
      document.title = 'WebistzuPDF - PDF Tools';
  ...
  if (dom.toolGrid) {
    ...
    categories.forEach((category) => {
      ...
      title.className =
        'text-xl font-bold text-sky-400 mb-4 mt-8 first:mt-0 text-white';
      ...
      category.tools.forEach((tool) => {
        ...
        if (tool.href) {
          ...
          toolCard.className =
            'tool-card bg-slate-800 rounded-xl p-4 cursor-pointer flex flex-col items-center justify-center text-center hover:shadow-lg transition duration-200';
        } else {
        ...
        icon.className = `ph ${tool.icon} text-4xl mb-3 text-sky-400`;
  ...
  if (githubStarsElements.some((el) => el) && !__SIMPLE_MODE__) {
    fetch('https://api.github.com/repos/Zara-Datacenter/webistzupdf')
  ...
  if (shortcutsTabBtn && preferencesTabBtn) {
    shortcutsTabBtn.addEventListener('click', () => {
      preferencesTabBtn.classList.add('bg-sky-600', 'text-white');
